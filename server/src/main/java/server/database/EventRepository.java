...
public interface EventRepository extends JpaRepository<Event, Long>{
    ...
    @Query("UPDATE Event SET title = :title, " +
            "lastActivityDate = :lastActivityDate, inviteCode = :inviteCode, participants= :participants " +
            "WHERE id = :id")
    void modifyEvent(@Param("id") long id, @Param("title") String title, @Param("lastActivityDate") Date lastActivityDate,
                     @Param("inviteCode") String inviteCode, @Param("participants") List<Participant> participants);
